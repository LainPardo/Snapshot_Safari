---
title: "Script to convert Snapshot records to Darwin Core (DwC) Format for FBIP"
author: "Lain E. Pardo"
date: "6/6/2021"
...
This code converts an original Snapshot report to the Darwin Core (DwC) format used widely for biodiversity records and by FBIP in South Africa. Here we are including ALL columns from original reports as extra information.
...
sp_rec <- read_csv("data_in/MTZ_S1_full_report_0-50%_agreement_corrected_fin.csv") 
...
since we are  working from a "non-clean" report I have to do some data-prep before the actual function for DwC format (this is not an issue if working from clean independent reports)

## Data preparation
be mindful that this code eliminates birds and false triggers, if birds are need set FALSE the rm_bycatch argument (but probably some things won´t work e.g. scientific names). See other scripts in this repo

```{r}
source("1_SNAPSHOT_source_functions.R")

reps1 <- cols_snap_std(sp_rec) 
...
reps2 <- df %>% mutate(DateTimeOriginal = paste(capture_date_local, capture_time_local))
...
-------------
Check carefully the strings, values and class of your variables. For example, I had to run this for kgalagadiS2-S4 as site name does include the three letters code already!! so messing the code...

I have separated KHO and KGA into different files (same for DHP,OVE)
```{r}
#sp_rec$site <- gsub("KHO", "", as.character(sp_rec$site))
```
-------------

also there could be some discrepancies between names, some reports would have for example generic names for some specie e.g. zebra instead of zebraburchells, zebramountain, etc.
...
#AUG
#unique(sp_rec$question_question__species)
...
cols <- c("Institution ID",	"Collection ID",	"Dataset ID",	"Basis of Record",	"Catalogue Number",
          ...
          "Vernacular Name",	"Taxon Rank",	"Nomenclatural Status",	"Taxon Remarks", "Camera.Site",
          #let´s add all cols from reports as extra info too
          names(unique(sp_rec)))
                #"season", "roll",
        #  "zooniverse_url_0", "zooniverse_url_1", "zooniverse_url_2", "Camera.Site", )
...
myfile <- file.path("data_out", paste0(locality, "_FBIP_Form.csv")) #change if needed
...
## so, now  let´s run above function
...
note. don´t forget to double check if results make sense, and species or rows are not lost in the process. 
